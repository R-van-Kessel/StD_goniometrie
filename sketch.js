...
function generateQuestions() {
    ...
    for (let i = 0; i < 10; i++) {
        // DRIEHOEK LAYOUT (blijft altijd hetzelfde):
        // a = hoek linksboven
        // b = hoek rechtsonder (α hoek)
        // rechte hoek = linksonder (90°)
        // c = overstaande zijde (links, verticaal)
        // d = aanliggende zijde (onder, horizontaal)  
        // e = schuine zijde (diagonaal, hypotenusa)
        
        // Kies random welke 2 waardes BEKEND zijn, en welke 1 ONBEKEND is
        let questionType = floor(random(6));  // 6 verschillende types vragen
        
        let gegeven1 = "";  // Eerste gegeven (bijv. "b")
        let waarde1 = 0;    // Waarde van eerste gegeven (bijv. 30 voor hoek b=30°)
        let gegeven2 = "";  // Tweede gegeven
        let waarde2 = 0;    // Waarde van tweede gegeven
        let vraag = "";     // Wat moet berekend worden (bijv. "c")
        let answer = 0;     // Het antwoord
        
        if (questionType === 0) {
            // TYPE 0: Gegeven hoek b + zijde d, zoek zijde c
            // Formule: tan(b) = c/d  →  c = d × tan(b)
            let hoekB = floor(random(20, 70));          // Hoek tussen 20-70°
            let zijdeD = floor(random(5, 15));          // Aanliggende zijde
            let zijdeC = zijdeD * tan(radians(hoekB));  // Bereken overstaande
            ...
            waarde2 = zijdeD;
            vraag = "c";
            answer = round(zijdeC * 100) / 100;  // Afronden op 2 decimalen
            
        } else if (questionType === 1) {
            // TYPE 1: Gegeven hoek b + zijde e, zoek zijde c
            // Formule: sin(b) = c/e  →  c = e × sin(b)
            let hoekB = floor(random(20, 70));
            let zijdeE = floor(random(8, 18));          // Schuine zijde
            ...
            // TYPE 2: Gegeven hoek b + zijde e, zoek zijde d
            // Formule: cos(b) = d/e  →  d = e × cos(b)
            let hoekB = floor(random(20, 70));
            ...
            // TYPE 3: Gegeven zijde c + zijde d, zoek hoek b
            // Formule: tan(b) = c/d  →  b = arctan(c/d)
            ...
            waarde2 = zijdeD;
            vraag = "b";
            answer = round(hoekB);  // Graden, afgerond op heel getal
            
        } else if (questionType === 4) {
            // TYPE 4: Gegeven zijde c + zijde e, zoek hoek b
            // Formule: sin(b) = c/e  →  b = arcsin(c/e)
            let zijdeC = floor(random(4, 12));
            let zijdeE = zijdeC + floor(random(3, 8));  // e moet groter dan c
            ...
            // TYPE 5: Gegeven zijde d + zijde e, zoek hoek b
            // Formule: cos(b) = d/e  →  b = arccos(d/e)
        ...
        // Sla vraag op
        questions.push({ 
            text: '',  // Geen tekst, wordt getekend
            answer: answer,
            triangleData: {
                gegeven1: gegeven1,
                waarde1: waarde1,
    ...
    for (let row = 2; row < 4; row++) {
        for (let col = 0; col < COLS; col++) {
            let answerText = "";
            let vraagLetter = questions[answerIndex % 10].triangleData.vraag;
            
            // Is het een hoek (a of b) of zijde (c, d, e)?
            if (vraagLetter === "a" || vraagLetter === "b") {
                answerText = vraagLetter + " = " + answers[answerIndex] + "°";
            } else {
                answerText = vraagLetter + " = " + answers[answerIndex];
            }
...
// ===== TEKEN FUNCTIE VOOR BLOKJE =====
function drawBlock(block) {
    // Bepaal kleur
    if (isChecked && block.isCorrect !== null) {
        if (block.isCorrect) {
            fill(100, 200, 100);  // Groen = correct
        } else {
            fill(250, 100, 100);  // Rood = fout
        }
    } else if (block.isQuestion) {
        fill(100, 150, 250);  // Blauw = vraag
    } else {
        fill(255, 200, 100);  // Oranje = antwoord
    }
...
function drawTriangleQuestion(block) {
    ...
    let x = block.x + 20;  // ← AANGEPAST: Was 10, nu 20 (meer naar rechts)
    let y = block.y + CELL_SIZE / 2;  // Verticaal gecentreerd
    
    push();
    translate(x, y);
    
    // ===== TEKEN DRIEHOEK (altijd dezelfde orientatie) =====
    let size = 50;  // ← AANPASBAAR: Grootte driehoek
    
    // Coördinaten:
    let x1 = 0, y1 = size/2;      // Linksonder (rechte hoek)
    let x2 = size, y2 = size/2;   // Rechtsonder (hoek b)
    let x3 = 0, y3 = -size/2;     // Linksboven (hoek a)
    ...
    // Drie lijnen
    line(x1, y1, x2, y2);  // Onderkant (zijde d)
    line(x2, y2, x3, y3);  // Schuine zijde (zijde e)
    line(x3, y3, x1, y1);  // Linkerkant (zijde c)
    
    // Rechte hoek symbool (linksonder)
    stroke(255);
    noFill();
    rect(x1, y1 - 4, 4, 4);
    ...
    noStroke();
    textStyle(BOLD);
    textSize(11);  // ← AANPASBAAR: Lettergrootte labels
    
    // Hoek a (linksboven, IN de hoek)
    ...
    // Zijde c (links, verticaal)
    textAlign(RIGHT, CENTER);
    text("c", x1 - 3, (y1 + y3) / 3);  // ← AANGEPAST: Was -4, nu -3 (dichter bij lijn)
    
    // Zijde d (onder, horizontaal)
    ...
    // ===== GEGEVENS RECHTS VAN DRIEHOEK =====
    let infoX = block.x + 65;  // ← AANGEPAST: Was 50, nu 60 (meer naar rechts)
